...
### **Lesson 4: Step 1 - Target Identification**
...
**Action:**
1.  Go to a large chemical database like **PubChem** ([https://pubchem.ncbi.nlm.nih.gov/](https://pubchem.ncbi.nlm.nih.gov/)).
...
6.  Open a simple text editor and save this SMILES string in a file named `quercetin_smiles.txt` inside your `data` folder.
...
**Explanation:** Now we will use the powerful SwissTargetPrediction web server. It will take our SMILES string and, based on chemical similarity, predict the most likely human protein targets.

**Action:** 1. Go to the **SwissTargetPrediction** website (<http://www.swisstargetprediction.ch/>). 2. Paste the SMILES string for Quercetin into the search box. 3. Make sure the species is set to **"Homo sapiens"**. 4. Click **"Predict targets"**. 5. The server will run the analysis and return a results page. This page shows a list of protein targets, ranked by a "Probability" score. 6. We want to export this data. Click the **"CSV"** button to download the results as a comma-separated values file. 7. Save this file in your `data` folder as `quercetin_predicted_targets.csv`.
...
-   **Verification:** Open the downloaded CSV file in a spreadsheet program (like Excel or Google Sheets) or a text editor. You will see a clean table. The most important columns are:
    -   `Target`: The common name of the protein target (e.g., "Serine/threonine-protein kinase PIM1").
...
**Action:**
1.  Go to the **GeneCards** website ([https://www.genecards.org/](https://www.genecards.org/)).
2.  In the search bar, type **"colon cancer"** and press Enter.
3.  GeneCards will show you a results page. It should have a section or link for the "Gene list" associated with the disease. Click on it.
4.  This will take you to a large table of genes. Each gene has a "GCid" and a "Relevance Score" indicating how strongly it is associated with the disease.
5.  We want to download this data. Look for a "Download" button or link.
6.  Download the table as a text or CSV file.
7.  Save this file in your `data` folder as `colon_cancer_targets.csv`.
...
*   **Verification:** Open the downloaded CSV file. You will see a large table with many columns. The most important columns for us are:
    ...
    *   `Relevance score`: The score from GeneCards.
    This file is our raw list of disease-associated targets.
---
...
We have successfully completed the data collection phase. We now possess the two essential lists of "characters" for our story. The next logical step is to see where these two lists overlap.
